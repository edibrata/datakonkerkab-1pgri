    ...
    <div className="max-w-4xl mx-auto space-y-4 md:space-y-6 animate-in fade-in pb-12">
      {!selectedPerson ? (
        ...
        <div className="animate-in fade-in slide-in-from-bottom-6 duration-700">
          ...
          {/* Main Content Card */}
          <div className="bg-white rounded-[2rem] p-6 md:p-8 relative shadow-[0_2px_24px_rgb(0,0,0,0.04)] border border-slate-100 flex flex-col group max-w-3xl mx-auto mb-10">
            {/* Selected Participant Hero Section */}
            <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start mb-6 md:mb-8">
              <div className="bg-slate-50 p-1 rounded-[1.5rem] shadow-sm self-center md:self-start">
                  <PhotoAvatar photoStr={selectedPerson.foto} name={selectedPerson.name} sizeClass="w-32 h-32 md:w-36 md:h-36" roundedClass="rounded-[1.25rem] shadow-inner" />
              </div>
              
              <div className="flex-1 flex flex-col w-full items-center text-center md:items-start md:text-left">
                <div className="flex items-center justify-between mb-3 w-full">
                  <div className="inline-flex items-center bg-[#2859c5] text-white px-3 py-1 rounded-[0.5rem] font-bold text-[10px] md:text-xs tracking-wide shadow-sm uppercase">
                    {selectedPerson.room && selectedPerson.room !== "X" ? `KAMAR ${selectedPerson.room}` : "BELUM ADA KAMAR"}
                  </div>
                  <span className="text-slate-300 font-extrabold text-lg md:text-xl">#{selectedPerson.id ? selectedPerson.id.slice(-4).toUpperCase() : "1"}</span>
                </div>
                
                <h3 className="text-2xl md:text-[28px] font-black text-[#1e293b] mb-3 leading-tight tracking-tight">{selectedPerson.name}</h3>
                
                <div className="flex flex-col gap-1.5 text-sm md:text-[15px] font-bold text-slate-500 mb-5 items-center md:items-start w-full">
                  <div className="flex items-center text-[#64748b]">
                     <Building2 className="w-4 h-4 md:w-[18px] md:h-[18px] mr-2 text-[#94a3b8] flex-shrink-0" />
                     <span className="uppercase">{selectedPerson.branch}</span>
                  </div>
                  {selectedPerson.kom && selectedPerson.kom !== "-" && (
                     <div className="flex items-center text-[#2563eb]">
                        <Layers className="w-4 h-4 md:w-[18px] md:h-[18px] mr-2 flex-shrink-0" />
                        <span>{selectedPerson.kom}</span>
                     </div>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                   {selectedPerson.kategori && (
                      <div className="border border-slate-200 bg-white shadow-sm rounded-[0.5rem] px-3 py-1.5 text-xs font-bold text-slate-700 whitespace-nowrap">
                        {selectedPerson.kategori}
                      </div>
                   )}
                   {selectedPerson.jabatan && selectedPerson.jabatan !== "-" && (
                      <div className="border border-emerald-200 bg-emerald-50 text-emerald-700 rounded-[0.5rem] px-3 py-1.5 text-xs font-bold whitespace-nowrap">
                        Jabatan: {selectedPerson.jabatan}
                      </div>
                   )}
                   <a href={getWaLink(selectedPerson.wa, false)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 border border-[#25D366]/30 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#1da851] rounded-[0.5rem] px-3 py-1.5 text-xs font-bold transition-colors whitespace-nowrap">
            ...
            <div className="flex flex-col mt-4">
              <div className="flex items-center gap-2 mb-4 px-1">
                 ...
                 <h4 className="font-extrabold text-[#94a3b8] text-[15px]">Teman Sekamar</h4>
              </div>
              
              {roommates.length > 0 ? (
                <div className="flex flex-col gap-3">
                  {roommates.map((mate, idx) => (
                    <div key={idx} className="bg-[#f8fafc] rounded-2xl p-3 md:p-4 border border-slate-100 hover:border-slate-200 flex items-center gap-4 transition-all duration-300">
                      <PhotoAvatar photoStr={mate.foto} name={mate.name} sizeClass="w-12 h-12 md:w-14 md:h-14" roundedClass="rounded-full shadow-sm" />
                      
                      <div className="flex-1 min-w-0">
                         <p className="font-bold text-[#1e293b] text-sm md:text-base truncate mb-0.5">{mate.name}</p>
                         <p className="text-[10px] md:text-[11px] font-bold text-[#94a3b8] truncate uppercase tracking-widest">{mate.branch}</p>
                      </div>
                      
                      <a 
                        href={getWaLink(mate.wa, false)} target="_blank" rel="noopener noreferrer"
                        className="w-10 h-10 flex items-center justify-center rounded-full bg-white hover:bg-[#25D366]/10 text-[#25D366] transition-colors flex-shrink-0 shadow-sm border border-slate-100"
                        title="Chat WA"
                      >
                ...
                <div className="bg-[#f8fafc] rounded-2xl p-6 border border-slate-100 border-dashed text-center">
                  <Users className="w-8 h-8 text-slate-300 mx-auto mb-2" />
                  <p className="text-slate-500 font-bold text-sm">Belum ada rekan sekamar.</p>
                </div>
              )}
            </div>
          </div>
