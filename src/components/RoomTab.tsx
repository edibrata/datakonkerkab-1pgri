    ...
    <div className="max-w-4xl mx-auto space-y-4 md:space-y-6 animate-in fade-in pb-12">
      {!selectedPerson ? (
        ...
        <div className="animate-in fade-in slide-in-from-bottom-6 duration-700">
          ...
          <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-10 relative overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-200 mb-8 md:mb-10 flex flex-col md:flex-row gap-6 md:gap-10 items-center md:items-start group">
            <PhotoAvatar photoStr={selectedPerson.foto} name={selectedPerson.name} sizeClass="w-24 h-24 md:w-36 md:h-36 ring-4 ring-slate-50 shadow-xl shadow-slate-200/50" />
            
            <div className="flex-1 text-center md:text-left flex flex-col w-full md:mt-2">
              <div className="flex justify-center md:justify-start gap-2 md:gap-3 mb-4 md:mb-5 flex-wrap">
                <div className="inline-flex items-center bg-slate-900 px-3 md:px-4 py-1.5 rounded-lg shadow-sm">
                  <span className="text-[10px] md:text-xs font-bold tracking-widest uppercase text-slate-400 mr-2">Kamar</span>
                  <span className="text-xs md:text-base font-black text-white">{selectedPerson.room}</span>
                </div>
                {selectedPerson.kom && selectedPerson.kom !== "-" && (
                    <div className="inline-flex items-center bg-slate-100 px-2.5 md:px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-200 transition-colors">
                      <span className="text-[9px] md:text-xs font-bold tracking-wider text-slate-700 uppercase">{selectedPerson.kom}</span>
                    </div>
                )}
                {selectedPerson.kategori && (
                    <div className="inline-flex items-center bg-transparent border border-slate-200 px-2.5 md:px-3 py-1.5 rounded-lg">
                      <span className="text-[9px] md:text-xs font-bold tracking-widest uppercase text-slate-500">{selectedPerson.kategori}</span>
                    </div>
                )}
              </div>
              
              <h3 className="text-xl md:text-3xl font-black text-slate-900 mb-3 md:mb-4 leading-tight tracking-tight">{selectedPerson.name}</h3>
              
              <div className="flex flex-col gap-2 md:gap-3 items-center md:items-start text-xs md:text-sm text-slate-600 font-medium">
                <div className="flex items-center">
                    <MapPin className="w-3.5 h-3.5 md:w-4 md:h-4 mr-2 md:mr-2.5 text-slate-400" />
                    <span>{selectedPerson.branch}</span>
                </div>
                {selectedPerson.jabatan && selectedPerson.jabatan !== "-" && (
                    <div className="flex items-center">
                        <BadgeInfo className="w-3.5 h-3.5 md:w-4 md:h-4 mr-2 md:mr-2.5 text-slate-400" />
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-6 md:mb-8 pb-3 md:pb-4 border-b border-slate-100">
               <h3 className="text-lg md:text-2xl font-bold text-slate-900 flex items-center gap-2 md:gap-3 tracking-tight">
                  <Users className="w-5 h-5 md:w-6 md:h-6 text-slate-400" />
                  Rekan Sekamar
               </h3>
               <span className="text-xs md:text-sm font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full border border-slate-200">{roommates.length} Orang</span>
            </div>
            
            {roommates.length > 0 ? (
              <div className="grid grid-flow-row grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                {roommates.map((mate, idx) => (
                  <div key={idx} className="bg-white rounded-2xl md:rounded-3xl p-4 md:p-6 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center sm:items-start gap-4 md:gap-5 hover:shadow-lg transition-all duration-300">
                    <PhotoAvatar photoStr={mate.foto} name={mate.name} sizeClass="w-16 h-16 md:w-20 md:h-20 shadow-md ring-1 ring-slate-100" />
                    
                    <div className="flex-1 w-full text-center sm:text-left min-w-0 flex flex-col justify-between h-full">
                      <div className="mb-3 sm:mb-0">
                          <p className="font-bold text-slate-900 text-base md:text-lg mx-auto sm:mx-0 truncate mb-1 md:mb-1.5">{mate.name}</p>
                          <div className="flex flex-col items-center sm:items-start gap-1 md:gap-1.5 relative">
                             <p className="text-[11px] md:text-sm font-medium text-slate-500 truncate w-full">{mate.branch}</p>
                             {mate.kom && mate.kom !== "-" && (
                                 <span className="inline-block text-[9px] md:text-[10px] font-bold uppercase text-slate-600 bg-slate-100 px-2 py-1 rounded border border-slate-200 mt-1 tracking-wider whitespace-nowrap">
                                    {mate.kom}
                                 </span>
                             )}
                          </div>
                      ...
                      <div className="flex gap-2 mt-4">
                        <div className="group/tooltip relative flex-1">
                          <a 
                            ...
                            className="bg-[#25D366]/10 hover:bg-[#25D366] text-[#25D366] hover:text-white px-2.5 md:px-3 py-2 md:py-2.5 rounded-xl transition-all flex items-center justify-center w-full shadow-sm border border-transparent hover:border-[#25D366]"
                          >
                        ...
                        <div className="group/tooltip relative flex-1">
                          <a 
                            ...
                            className="bg-slate-50 hover:bg-slate-100 text-slate-600 hover:text-slate-900 px-2.5 md:px-3 py-2 md:py-2.5 rounded-xl transition-all flex items-center justify-center w-full shadow-sm border border-slate-200"
                          >
              ...
              <div className="bg-slate-50 rounded-2xl md:rounded-[2rem] p-8 md:p-12 border border-slate-200 border-dashed text-center flex flex-col items-center justify-center">
                <Users className="w-10 h-10 md:w-12 md:h-12 text-slate-300 mb-3 md:mb-4" />
                <p className="text-slate-600 font-medium text-base md:text-lg">Belum ada rekan sekamar yang dialokasikan.</p>
                <p className="text-xs md:text-sm text-slate-400 mt-2">Data teman sekamar akan muncul setelah diatur oleh panitia.</p>
              </div>
            )}
          </div>
        </div>
