...
export default function App() {
  ...
  // Modal States
  const [modalConfig, setModalConfig] = useState<{ title: string, message: string, type: 'success' | 'error', showProgress?: boolean } | null>(null);
  ...
  const showModal = (title: string, message: string, type: 'success' | 'error', showProgress?: boolean) => {
                ...
                <div className="overflow-auto max-h-[70vh]">
                     ...
                     {modalConfig.showProgress && (
                         <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden mb-4">
                             <div className="h-full gradient-bg transition-all duration-300" style={{ width: `${modalProgress}%` }}></div>
                         </div>
                     )}
                </div>
      ...
      {showCategoryModal && (
        <div className="fixed inset-0 z-[140] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md">
            <div className="bg-white w-full max-w-xl rounded-2xl p-6 md:p-8 shadow-2xl relative text-slate-800">
                <button onClick={() => setShowCategoryModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-red-600 transition-all">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div onClick={() => handleCategorySelect('PESERTA CABANG')} className="category-card border border-slate-100 rounded-xl p-5 flex flex-col items-center text-center cursor-pointer bg-white shadow-sm hover:shadow-md">
                        <div className="w-12 h-12 bg-red-50 text-red-600 rounded-full flex items-center justify-center mb-3">
                            <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
                        </div>
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md">
            <div className="bg-white w-full max-w-xl rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                <div className="gradient-bg px-4 py-4 md:px-6 md:py-6 text-white text-center">
                    ...
                    <p className="text-[9px] md:text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1">KONFERENSI KERJA 1 PGRI KABUPATEN PANDEGLANG 2026</p>
                </div>
                <div className="p-4 md:p-6 overflow-y-auto custom-scrollbar flex-grow bg-white text-slate-700 space-y-3 md:space-y-4">
                    <div className="space-y-3 text-[11px] leading-relaxed">
                        <div className="bg-slate-50 p-3 md:p-4 rounded-lg border border-slate-100">
                            <p className="font-black text-slate-900 mb-1.5 md:mb-2 uppercase">1. Ketentuan Umum</p>
                            <ul className="list-disc ml-4 space-y-1 font-medium">
                                <li>Pendaftaran dilakukan secara online melalui portal resmi Konkerkab 1 PGRI.</li>
                                <li>Pilih kategori pendaftaran <span className="font-bold text-red-600">Peserta Cabang</span>.</li>
                                <li>Setiap Cabang mengikutsertakan <span className="font-bold">4 orang peserta</span> yang dibuktikan dengan Surat Mandat resmi cabang.</li>
                            </ul>
                        </div>
                        <div className="bg-slate-50 p-3 md:p-4 rounded-lg border border-slate-100">
                            <p className="font-black text-slate-900 mb-1.5 md:mb-2 uppercase">2. Persiapan Data (Siapkan sebelum mengisi):</p>
                            <ul className="list-disc ml-4 space-y-0.5 md:space-y-1 font-medium">
                                ...
                                <li>Ukuran kaos masing-masing delegasi (S, M, L, XL, XXL, XXXL).</li>
                                <li>Pas foto formal memakai batik PGRI (Wajah jelas, tidak blur, posisi tegak).</li>
                                <li>Soft file <span className="font-bold">Surat Mandat kolektif</span> (Scan jelas, stempel resmi).</li>
                            </ul>
                        </div>
                        <div className="bg-slate-50 p-3 md:p-4 rounded-lg border border-slate-100">
                            <p className="font-black text-slate-900 mb-1.5 md:mb-2 uppercase">3. Pengaturan Google Drive (Wajib):</p>
                            <ul className="list-disc ml-4 space-y-1 font-medium">
                                <li>Simpan Surat Mandat di Google Drive masing-masing cabang.</li>
                                <li>Atur akses menjadi: <span className="font-bold text-emerald-600">"Siapa saja yang memiliki link dapat melihat"</span>.</li>
                                <li>Salin tautan (link) tersebut untuk diisikan pada formulir pendaftaran.</li>
                            </ul>
                        </div>
                        <div className="bg-slate-50 p-3 md:p-4 rounded-lg border border-slate-100">
                            <p className="font-black text-slate-900 mb-1.5 md:mb-2 uppercase">4. Informasi Pembayaran:</p>
                            <div className="p-3 bg-white border border-red-100 rounded text-center relative group">
                                <p className="text-[10px] font-bold text-slate-400 uppercase">Bank BJB (Giro)</p>
                                <div className="flex items-center justify-center gap-2 my-0.5">
                                    <p className="text-sm font-black text-slate-900">0011752268001</p>
                                    <button onClick={() => {
                                        navigator.clipboard.writeText('0011752268001');
                                        const badge = document.getElementById('copyBadgeReact');
                                        if (badge) {
                                            badge.classList.remove('hidden');
                                            setTimeout(() => badge.classList.add('hidden'), 2000);
                                        }
                                    }} className="text-slate-400 hover:text-red-600 transition-all tooltip-container cursor-pointer">
                                        <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                                        <span className="tooltip-text">Salin No. Rek</span>
                                    </button>
                                </div>
                                <p className="text-[9px] font-bold text-red-600 uppercase">A.N: PGRI Kabupaten Pandeglang</p>
                                <div id="copyBadgeReact" className="hidden absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[8px] px-2 py-1 rounded shadow-lg animate-bounce">TERSALIN!</div>
                            </div>
                        </div>
                        <div className="bg-slate-50 p-3 md:p-4 rounded-lg border border-slate-100">
                            <p className="font-black text-slate-900 mb-1.5 md:mb-2 uppercase">5. Selesai Pendaftaran:</p>
                            <ul className="list-disc ml-4 space-y-1 font-medium">
                                <li>Pastikan data benar sebelum menekan <span className="font-bold">"Simpan & Cetak ID Card"</span>.</li>
                                <li>Data tersimpan dan ID Card akan terunduh otomatis dalam format PDF.</li>
                                <li>Gunakan <span className="font-bold">Token Revisi</span> pada bukti pendaftaran jika ada kesalahan data di kemudian hari.</li>
                            </ul>
                        </div>
                    </div>
                </div>
